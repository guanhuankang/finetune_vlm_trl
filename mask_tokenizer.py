...
class MaskTokenizer:
    ...
    def encode2code(self, mask_np):
        assert np.max(mask_np) <= 1.0 and len(mask_np.shape) == 2
        mask_3c = np.repeat(
            mask_np[..., None], 3, axis=-1
        )  # or np.stack([mask_np]*3, axis=-1)
...
if __name__ == "__main__":
    ...
    mt = MaskTokenizer(
        checkpoint_path="assets/1d_tokenizer/tokenizer_titok_l32_imagenet"
    )
    mask_rle = {
        "size": [480, 640],
        "counts": "]UZ34k>2Ld0]O2N0PBUOP>f072N40L3M4L=_OVVl5",
    }
    mask_np = coco_mask_decode(mask_rle)
    x, y, w, h = tuple(map(int, [226.0, 229.0, 12.0, 31.0]))
    mask = mask_np[y:y+h, x:x+w]

    code = mt.encode2code(mask)
    output = mt.code2mask(code, size=mask.shape)

    print(code.view(-1).tolist())
    Image.fromarray(mask * 255).save("output/mt_input.jpg")
